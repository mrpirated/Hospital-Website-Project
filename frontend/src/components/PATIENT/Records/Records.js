...
export default function Records() {
    ...
    useEffect(() => {
        if (!(auth.isauth && auth.type === 0)) {
            ...
            patientAllAppointmentsAPI({
                ...
                var now = new Date();
                if (res.reply) {
					var tmp1 = res.appointments.filter((item) => (

                        new Date(item.start_time) >= now
                    ))
                    console.log(tmp1);
                    var tmp2 = res.appointments.filter((item) => (
                        new Date(item.start_time) < now
                    ))
                    console.log(tmp2);
                    setPast(tmp2);
    ...
    return (
        <div>
            <TabComponent tabList={TabNav} selectedKey="future" all={all} past={past} future={future} />
        </div>
    )
}
