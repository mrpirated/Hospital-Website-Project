...
import { Card, ListGroup, ListGroupItem, Image, DropdownButton, Dropdown, Row, Col } from "react-bootstrap";
...
function Doctors() {
	...
	return (
		<div>
										...
										{/* <ListGroup style={{ marginTop: "3%", fontSize: "0.6rem" }}> */}
											{
												d.specialization.map((s) => (
													<p style={{display:"inline-block", margin: "0", padding: "5px", marginLeft:"5px", backgroundColor:"black", color:"white", borderRadius:"8px"}}>{s}</p>
												))
											}
										{/* </ListGroup> */}
									</ListGroupItem>
			...
			<div className="pagination" style={{paddingBottom: "20px", paddingTop: "20px"}}>
				{/* previous button */}
				<button
				...
				{getPaginationGroup().map((item, index) => (
					<button
						key={index}
						onClick={changePage}
						className={`${index >= pages ? 'disabled' : ''} paginationItem ${currentPage === item ? 'active' : null}` }
