				...
				if (response.success) {
					...
					for(let i = 0; i < response.data.appointments.length; i++){
						...
						tmp.push({
							...
							Description:"DESCRIPTION: " + response.data.appointments[i].case_description + ", " +
										"CASE ID: " + response.data.appointments[i].case_id + ", " +
										"STATE: " + response.data.appointments[i].state,
